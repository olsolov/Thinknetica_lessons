...
class Main
  ...
  def start
    loop do
      ...
      when '11'  
        take_seet_by_train_wagon
      when '12'
        take_volume_by_train_wagon
      else
  ...
  def menu
    ...
    puts 'Введите 11, если вы хотите занять место в вагоне'
    puts 'Введите 12, если вы хотите занять объем в вагоне'
  ...
  def show_stations_info
    Station.all.each do |station|
      if station.trains.size == 0
        ...
        station.trains_by_station {|train| puts "Поезд № #{train.number}, тип: #{train.type}, кол-во вагонов: #{train.train_wagons.count}"}
      end
    end
  end
  ...
  def show_trains_by_station
    ...
    choice_station.trains.each do |train|
      ...
      if train.type == :passenger
        train.wagons_by_trains {|wagon| puts "№ вагона: #{rand(10..99)}, тип: #{wagon.type}, кол-во свободных/занятых мест: #{wagon.vacant_seats}/#{wagon.busy_seats}"}
      elsif train.type == :cargo
        train.wagons_by_trains {|wagon| puts "№ вагона: #{rand(10..99)}, тип: #{wagon.type}, кол-во свободного/занятого объема: #{wagon.available_volume}/#{wagon.busy_volume}"}
      end
    end
  end

  def take_seet_by_train_wagon
  ...
  def take_volume_by_train_wagon
    puts 'Введите номер поезда, в котором вы хотите занять объем: '
    show_trains
    train_number = gets.strip
    choice_train = Train.find(train_number)
      if choice_train.type == :cargo
        puts 'Введите порядковый номер вагона, в котором вы хотите занять объем: '
        choice_train.train_wagons.each_with_index do |wagon, index|
          puts "#{index +=1}. Вагон: #{wagon.type}"
        end
        answer = gets.to_i
        ...
        puts "Вы заняли #{volume} кубов в поезде № #{choice_train.number}, в #{answer} вагоне"
      else
